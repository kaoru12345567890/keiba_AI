...
print("【条件適用】中央10競馬場すべてのデータを使用します（函館・札幌のフィルタリングを解除）...")
...
# 2. 超・函館札幌特化型の重み付けルール
# ==========================================
print("中央10場拡張に伴う、洋芝開催（函館・札幌）の超特化重み付け処理中...")
max_year = df['年'].max()
...
def evaluate_and_print_results(test_df, target_year):
    ...
    if not yo_shiba_df.empty:
        ...
        print(f"\n※ {target_year}年のテストデータに 函館・札幌開催 のレースは含まれていません。")
...
for stage_y in range(2011, 2016):
    ...
    # 【リークなき完全対応】この段階の過去データ（stage_tr）のみからターゲットエンコーディングを算出
    init_stats_frame_course = stage_tr.groupby('course_frame_key')['着順'].apply(lambda x: (x <= 3).mean())
    init_stats_course_style = stage_tr.groupby('course_style_key')['着順'].apply(lambda x: (x <= 3).mean())
    init_stats_jockey_place_turf_dirt = stage_tr.groupby('騎手_競馬場_芝ダート')['着順'].apply(lambda x: (x <= 3).mean())
    init_stats_trainer_place_turf_dirt = stage_tr.groupby(['調教師名', '場所', '芝ダート'])['着順'].apply(lambda x: (x <= 3).mean())
    init_baseline_rate = (stage_tr['着順'] <= 3).mean()

    # 仕込みループ用の一時データフレームにマッピング
    for t_df in [stage_tr, stage_va]:
        t_df['course_frame_rate'] = t_df['course_frame_key'].map(init_stats_frame_course).fillna(init_baseline_rate)
        t_df['course_style_rate'] = t_df['course_style_key'].map(init_stats_course_style).fillna(init_baseline_rate)
        t_df['jockey_place_turf_dirt_rate'] = t_df['騎手_競馬場_芝ダート'].map(init_stats_jockey_place_turf_dirt).fillna(init_baseline_rate)
        t_df['trainer_key'] = list(zip(t_df['調教師名'], t_df['場所'], t_df['芝ダート']))
        t_df['trainer_place_turf_dirt_rate'] = t_df['trainer_key'].map(init_stats_trainer_place_turf_dirt).fillna(init_baseline_rate)

    # ターゲットエンコーディング特徴量をベース特徴量に合流させる
...
for target_year in tqdm(years, desc="本番ローリング実行中"):
    ...
    # 【★改造の核心】新しい年のテストを開始する際、以前のメタモデルを完全に棄却してインスタンスを新調します。
    # それまでの年（仕込み＋通過した本番の過去データ）のプールを使って最初から一発で綺麗に学習させます。
    ...
    # 予測が終わった後に、そのターゲット年の予測値プールを追加して、次の年の「一発再学習」に備えます（ここでの上書きフィットは廃止）
