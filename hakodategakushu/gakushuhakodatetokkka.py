...
# 地方競馬などのノイズを防ぐため、主要な中央10競馬場に限定します
...
def calculate_complex_weight_v3(row):
    ...
    # ★ 函館と札幌の重み付けを劇的に強化（かつ札幌を函館に肉薄する比率に調整）
    if '函館' in place:
        weight *= 10.0
    elif '札幌' in place:
        weight *= 8.5
    else:
        weight *= 1.0  # 他の8つの中央競馬場は基礎体力作りのベースとして等倍
...
# ==========================================
# 7. 5つのモデルを評価するレポート関数（JRA全10場 ＆ 函館・札幌 抽出表示）
# ==========================================
...
for target_year in tqdm(years, desc="本番ローリング実行中"):
    ...
    # 【レポート出力】中央全体の結果と、函館・札幌を抽出した結果を両方とも表示します
