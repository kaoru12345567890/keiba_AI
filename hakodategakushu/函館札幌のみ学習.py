...
# ------------------------------------------
# 【仕込みフェーズ】2010〜2015年の6年分で最初のメタモデルの土台を作る（函館・札幌特化＆５：３）
# ------------------------------------------
...
init_base = df[(df['年'] >= 2010) & (df['年'] <= 2015)].copy()
df_hakodate = init_base[init_base['場所'].isin(hakodate_labels)]
df_sapporo = init_base[init_base['場所'].isin(sapporo_labels)]
...
if (n_hakodate * 3 / 5) <= n_sapporo:
    ...
    sample_hakodate_n = int(n_sapporo * 5 / 3)
    sample_sapporo_n = n_sapporo
...
for stage_y in range(2011, 2016):
    ...
    tmp_rank = lgb.LGBMRanker(n_estimators=150, objective='lambdarank', **lgb_params).fit(
        stage_tr[features_tmp], 19 - stage_tr['着順'].clip(upper=18), group=stage_tr.groupby('レースキー').size().to_numpy()
    )
    tmp_class = lgb.LGBMClassifier(n_estimators=150, objective='binary', **lgb_params).fit(
        stage_tr[features_tmp], (stage_tr['着順'] <= 3).astype(int)
    )
    tmp_reg = lgb.LGBMRegressor(n_estimators=150, objective='regression', **lgb_params).fit(
        stage_tr[features_tmp], stage_tr['着順']
    )
    
    scaler_tmp = MinMaxScaler()
    r_p = scaler_tmp.fit_transform(tmp_rank.predict(stage_va[features_tmp]).reshape(-1, 1)).flatten()
    c_p = tmp_class.predict_proba(stage_va[features_tmp])[:, 1]
    reg_p = scaler_tmp.fit_transform((-tmp_reg.predict(stage_va[features_tmp])).reshape(-1, 1)).flatten()
