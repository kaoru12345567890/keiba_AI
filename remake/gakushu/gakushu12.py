...
importance_df = pd.DataFrame({'feature': current_features, 'importance': avg_importance}).sort_values('importance', ascending=False)

print("\n=== AIが重視している特徴量（重要度ランキング） ===")
print(importance_df.head(15))

top3 = final_results[final_results['rank'] <= 3]
is_hit = (top3['着順'] <= 3).mean()
print(f"\n=== 最終モデル：ランキング3位以内の複勝的中率: {is_hit:.2%} ===")