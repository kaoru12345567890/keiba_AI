...
# 3. 強制的な数値変換処理
cols_to_numeric = ['着順', '馬体重', '体重増減', '斤量', '単勝', '賞金']
for col in cols_to_numeric:
    df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

# 4. 前処理
drop_cols = ['レースID', 'レース名', '馬名', '馬主', 'タイム', '着差', '通過順', '賞金', '性別年齢', '人気', '上り3ハロン']
...
for target_year in tqdm(test_years, desc="学習進捗"):
    ...
    for t_col in target_cols:
        lgb_train = lgb.Dataset(train_sub.drop(columns=['着順', 'is_1st', 'is_top2', 'is_top3']), train_sub[t_col])
        lgb_eval = lgb.Dataset(val_sub.drop(columns=['着順', 'is_1st', 'is_top2', 'is_top3']), val_sub[t_col], reference=lgb_train)
        ...
        pred_label = (test_df[f'pred_{t_col}'] > 0.5).astype(int)
        auc = roc_auc_score(test_df[t_col], test_df[f'pred_{t_col}'])
        precision = precision_score(test_df[t_col], pred_label, zero_division=0)
        
        eval_results.append({'年': target_year, 'ターゲット': t_col, 'AUC': round(auc, 4), '的中率': round(precision, 4)})
    
    all_preds.append(test_df)

# 6. 結果の結合と保存
final_results = pd.concat(all_preds)
final_results.to_csv('predicted_results.csv', index=False)
pd.DataFrame(eval_results).to_csv('accuracy_report.csv', index=False)
...
plt.hist(final_results['pred_is_1st'], bins=50, color='skyblue', edgecolor='black', alpha=0.7)
plt.title('Distribution of Predicted Probabilities (pred_is_1st)')
