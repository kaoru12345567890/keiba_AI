...
# 【重要】もしヘッダー行そのものがデータとして含まれている場合、除外する
df = df[df['年'] != '年'].copy()
...
# 全ての数値を対象に変換（ここがポイント）
numeric_cols = [
    '月', '日', '回', '日目', 'レース目', '距離', '出走数', '着順', '枠番', '馬番', 
    '斤量', '馬体重', '体重増減', '賞金', '過去出走回数', '過去平均着順', '過去連対率', '過去複勝率', '過去平均上がり偏差値'
]
...
# 3. 特徴量の固定化（ペースを除外）
drop_cols = [
    'レースID', 'レース名', '馬名', '馬主', 'タイム', '着差', '通過順', '賞金', 
    '性別年齢', '上り3ハロン', '単勝', '人気', '着順', 'ペース'
]
df = df.drop(columns=drop_cols, errors='ignore')

# カテゴリ変数処理（数値以外をLabelEncoderで数値化）
...
for target_year in tqdm([y for y in years if y >= 2021], desc="学習進捗"):
    ...
    # ここで確実に数値型のみを選択
    X_train = train_full.iloc[:split_idx][feature_cols].apply(pd.to_numeric, errors='coerce').fillna(0)
    X_val = train_full.iloc[split_idx:][feature_cols].apply(pd.to_numeric, errors='coerce').fillna(0)
