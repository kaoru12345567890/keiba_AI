...
for target_year in tqdm(years, desc="学習進捗"):
    ...
    for t_df in [train_df, test_df]:
        ...
        # 2乗して強調する（重み付け）
        t_df['jockey_cond_rate_sq'] = t_df['jockey_cond_rate'] ** 2
        t_df['trainer_place_rate_sq'] = t_df['trainer_place_rate'] ** 2
        t_df['jockey_dist_rate_sq'] = t_df['jockey_dist_rate'] ** 2
    ...
    current_features = feature_cols + [
        ...
        'jockey_cond_rate_sq', 'trainer_place_rate_sq', 'jockey_dist_rate_sq',
        '斤量_rel', '過去平均着順_rel', '人気_rel'
    ]
...
print(f"\n=== 最適化完了！ランキング3位以内の複勝的中率: {is_hit:.2%} ===")