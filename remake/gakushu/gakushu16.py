...
df['track_condition'] = df['芝ダート'].astype(str) + '_' + df['馬場状態'].astype(str) + '_' + df['天気'].astype(str)
df['jockey_trainer'] = df['騎手'].astype(str) + '_' + df['調教師名'].astype(str)

le_cols = ['天気', '馬場状態', 'レース条件', '芝ダート', '回り', '騎手', '調教師名', '所属', '性別', '場所', 'course_key', 'track_condition', 'jockey_trainer']
...
print("--- 学習開始：相互作用特徴量追加による重要度の平滑化 ---")
for target_year in tqdm(years, desc="学習進捗"):
    ...
    stats_jockey_trainer = get_smoothed_rate(train_df, ['jockey_trainer'])
    ...
    current_features = [
        ...
        '馬体重', 'weight_ratio', 'weight_diff', 'popularity_vs_ability', 'weight_age_ratio',
        'course_key', 'track_condition', 'jockey_cond_rate', 'trainer_place_rate', 'jockey_trainer_rate'
    ]
