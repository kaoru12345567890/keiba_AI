...
# 3. 前処理 (数値変換・ラベルエンコーディング)
cols_to_numeric = ['着順', '馬体重', '体重増減', '斤量', '単勝', '賞金', '人気']
...
# B. 函館適性の特徴量（函館の過去平均着順）
# ※注意: 学習時はデータ漏洩を防ぐため、そのレースより過去のデータのみで計算する処理が必要です
# ここでは簡易的に過去の函館成績を特徴量として埋め込む準備をします
df['is_hakodate'] = (df['場所'] == 2).astype(int)

# 5. 学習処理
drop_cols = ['レースID', 'レース名', '馬名', '馬主', 'タイム', '着差', '通過順', '賞金', '性別年齢', '上り3ハロン', '単勝']
...
print("\n--- アップグレード後の学習完了 ---")
# 予測結果の確認：人気ランクと予測値の関係を見てみる
print(final_results[['人気', 'pred_is_1st']].head(10))