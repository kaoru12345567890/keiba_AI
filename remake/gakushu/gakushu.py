...
# 6. 結果の保存
pd.concat(all_preds).to_csv('predicted_results.csv', index=False)
eval_df = pd.DataFrame(eval_results)
eval_df.to_csv('accuracy_report.csv', index=False)

print("\n--- 年ごとの予測精度検証結果 ---")
print(eval_df)
print("\n完了: 予測結果は predicted_results.csv、精度レポートは accuracy_report.csv に保存されました。")
