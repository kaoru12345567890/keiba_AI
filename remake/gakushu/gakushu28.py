...
def evaluate_detailed(df_test, score_col):
    ...
    def get_metrics(x):
        ...
        # 3連複ボックス(4頭)の的中判定: 実際の3頭すべてがtop4に含まれているか
...
# 学習ループの最後に（all_results を concat した後で実行）
# 今回の学習で作成された最終モデル（trainデータの全量で再学習したもの）を保存します
print("--- モデルの保存を開始します ---")
final_train = df[df['年'] < 2026].copy() # 直近までの全データで最終学習
# ここで m_rank, m_class, m_reg を再度学習させて保存
