...
def add_features(t_df):
    ...
    for col in ['weight_ratio', '斤量', '過去平均着順', 'popularity_vs_ability','年齢','full_weight']:
    ...
    #   完全にiのループが終わった後で】同型ライバル頭数を一気に計算する
    t_df['同型ライバル頭数'] = 0
    for i in [1, 2, 3, 4]:
        condition = t_df['脚質ラベル'] == i
        t_df.loc[condition, '同型ライバル頭数'] = t_df.loc[condition, f'脚質{i}_頭数'] - 1
    ...
    t_df['騙馬年齢'] = t_df['年齢_rel'].astype(str)+ '_' + t_df['is_senba'].astype(str)
    t_df['牝馬年齢'] = t_df['年齢_rel'].astype(str)+ '_' + t_df['is_female'].astype(str)
    t_df['牡馬年齢'] = t_df['年齢_rel'].astype(str)+ '_' + t_df['is_male'].astype(str)
...
def evaluate_and_print_results(test_df, target_year):
    ...
    if not local_df.empty:
        ...
        # 上位3頭の抽出と的中確認
        top3_pred = local_df.groupby('レースID').head(3)
...
for target_year in tqdm(years, desc="年度別学習"):
    ...
    current_feats = feat_base + ['course_frame_rate', 'course_style_rate', 'jockey_place_turf_dirt_rate', 'trainer_place_turf_dirt_rate']
